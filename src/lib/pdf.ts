...
export function generateBillPDF(bill: Bill, settings: Settings): jsPDF {
  ...
  doc.setFont("helvetica", "normal");
  doc.text(bill.invoiceNumber || bill.id.slice(0, 16).toUpperCase(), leftX + 28, y);
  y += 5;
  doc.setFont("helvetica", "bold");
  ...
  if (bill.dueDate) {
    y += 5;
    doc.setFont("helvetica", "bold");
    doc.text("Due Date:", leftX, y);
    doc.setFont("helvetica", "normal");
    doc.text(new Date(bill.dueDate).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }), leftX + 28, y);
  }
  ...
  if (bill.notes) {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    const noteLines = doc.splitTextToSize(bill.notes, CW);
  ...
  doc.text(`Invoice: ${bill.invoiceNumber || bill.id.slice(0, 8).toUpperCase()}`, W - M, H - M, { align: "right" });
